...
window.addEventListener('load', function () {
    ...
    $('#bg').css("cssText", "transform: scale(1);filter: blur(0px);transition: ease 0.1s;");
    $('.cover').css("cssText", "opacity: 1;transition: ease 0.1s;");
...
$('#switchmore').on('click', function () {
    ...
    if (shoemore && $(document).width() >= 990) {
        ...
        $("#change").html("Oops&nbsp;!");
        $("#change1").html("哎呀，这都被叼毛发现了（ 再点击一次可关闭 ）");
    } else {
        $('#container').attr('class', 'container');
        $("#change").html("Hello&nbsp;World&nbsp;!");
        $("#change1").html("进恩哥资源仓库，仅供参考使用，请于24小时内将本站APP从您的设备进行卸载/删除操作。");
    }
});
